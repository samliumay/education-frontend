  ...
  <div class="flex flex-col justify-between gap-4 rounded-[12px] bg-white" data-cms="product-card">
    <div class="flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-1">
        ...
        <h2 class="text-2xl md:text-4xl font-medium uppercase leading-tight" :class="isGreenText ? 'text-brand-green' : 'text-brand-red'">
          {{ blockData.name }}
        </h2>
    ...
    <div class="flex flex-col gap-4">
      <div class="flex flex-col gap-2">
        <template v-if="blockData.product_type !== 'Academy'">
          <div v-for="item in schedule" :key="item.id" class="flex gap-4 items-center">
      ...
      <div>
        ...
        <AppButton
          ...
          @click="navigateTo(`/product/buy/${props.blockData.slug}`)"
        >
          {{ $t('common.actions.buyAcademia') }}
        </AppButton>

        <!-- Try Free button if the user is not logged in -->
        <AppButton
          v-else
          class="justify-start mt-4 mr-4 !bg-brand-red text-white"
          @click="openModal"
        >
          {{
            type === 'special'
        ...
        <!-- More details button -->
        <AppButton class="mt-[24px] !bg-brand-light-gray cursor-pointer" @click="goToDetails">
...
// Function to open the correct modal based on the type
const openModal = () => {
...
const goToDetails = (isOpenSignIn: boolean) => {
  router.push(`/product/${props.blockData?.slug || props.blockData?.id}`)
}
...
const categories = computed(() => {
  ...
  if (typeof props.blockData?.language === 'string' && props.blockData.language) {
    bufferArray.push(props.blockData.language.toLocaleUpperCase())
  }
  if (typeof props.blockData?.category === 'string' && props.blockData?.category) {
...
const tags = computed(() => {
  const bufferArray = []
  if (typeof props.blockData?.age_group === 'string' && props.blockData?.age_group) {
    bufferArray.push(`${props.blockData?.age_group} ${t('common.age')}`)
  }
  if (typeof props.blockData?.current_status === 'string' && props.blockData?.current_status) {
...
const schedule = computed(() => {
  ...
  props.blockData.schedule_slots.forEach(element => {
    bufferArray.push({
      start: `${t(`common.weekdays.short.${element.weekday.toLowerCase()}`)} ${getNearDate(element.weekday, element.start)}`,
