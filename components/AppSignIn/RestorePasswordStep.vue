<template>
  <form ref="form" @submit.prevent="restore">
    <AppInput v-model="email" placeholder="E-mail" type="email" required @blur="checkValidity" />
    <button class="mt-[16px] text-brand-red cursor-pointer" @click="$emit('goToEmailStep')">
      Я вспомнил пароль
    </button>

    <p v-if="error" class="text-brand-red mt-2 mb-2">{{ error }}</p>
    <p v-if="isSended" class="text-brand-gray mt-2 mb-2">
      Мы отправили вам сообщение для восстановления пароля. Проверьте,
      пожалуйста, вашу почту
    </p>

    <AppButton class="block mt-[36px] w-full" type="submit" :disabled="!form?.checkValidity() ?? false">
      Восстановить
    </AppButton>
  </form>
</template>
